...
class TDPG_Critic(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_layers = nn.Sequential(
            *[
                transformer_block(
                    num_heads,
                    inner_embed_size,
                    history_len + 1,   # add 1 for action embedd
                    dropout,
                    attn_gate,
                    mlp_gate,
                )
        ...
        self.ffn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(inner_embed_size, 1), # predict batch of q-values 
        )
        self.action_embedding = torch.nn.Linear(1, inner_embed_size)
    ...
    def forward(self, obss: torch.Tensor, act: torch.Tensor) -> torch.Tensor:
        '''
            act: has lenght of batch 
                return: q-value based on action, lenght: [batch x ]
        '''
        ...
        action_embeddings = self.action_embedding(act)

        mixed_embeddings = torch.cat(token_embeddings, action_embeddings, dim = 1) 

        # duplicate position embed for action - should I do this? 
        mixed_pos_embeddings = torch.cat(self.position_embedding[:, :history_len, :], self.position_embedding[:, history_len:history_len + 1, :], dim = 1)

        # batch_size x hist_len + 1 x obs_dim
        #x = self.dropout(token_embeddings + self.position_embedding[:, :history_len, :])
        x = self.dropout(mixed_embeddings + mixed_pos_embeddings)
...
class TDPG(nn.Module):
    ...
    def __init__(
        ...
        # init actir and ctritic networks
        self.actor = TDPG_Actor(
            obs_dim,
            action_dim,
            embed_per_obs_dim,
            inner_embed_size,
